...
const Q_TAXES = `
query Taxes($businessId: ID!) {
  business(id: $businessId) {
    id
    name
    salesTaxes(page:1, pageSize:100) {
      edges {
        node { id name rate archived }
      }
    }
  }
}`;
...
export default async function handler(req, res) {
  ...
  try {
    const data = await wave(Q_TAXES, { businessId }, token);
    const taxes = data?.business?.salesTaxes?.edges?.map(e => e.node) || [];
    return res.json({ ok:true, business: data.business?.name, taxes });
  } catch (err) {
    console.error('[wave/list-taxes] error:', err);
